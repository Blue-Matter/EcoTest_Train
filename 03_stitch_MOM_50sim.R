...
MOM_byc <- lapply(paste0('MOM/MOM_', byc, '_50sim.rds'), readRDS)
MOM_targ <- lapply(paste0('MOM/MOM_', targ, '_50sim.rds'), readRDS)
...
lapply(MOM1@cpars,function(x)print(x[[1]]$Mat_age[1,,1]))
#lapply(args,function(x)x@cpars[[1]][[1]]$Len_age[1,,1])
lapply(MOM1@cpars,function(x)print(x[[1]]$Len_age[1,,1]))

MOM2 = MOM_simplify(MOM1)
...
Hist = readRDS('MOM/multiHist_stitch_50sim.rds')
