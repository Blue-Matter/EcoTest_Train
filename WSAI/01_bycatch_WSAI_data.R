...
#### Catch
cat_W <- t1 %>% dplyr::filter(SampAreaCode %in% W)

# By gear - predominantly longline
cat_W %>% 
...
cat_W %>%
  ...
  ggplot(cat_W, aes(YearC, catch, colour = FlagName)) + facet_wrap(~GearGrp, scales = "free_y") + 
  geom_point() + geom_line()

## Longline by flag - predominantly Venezuela, Spain, Brazil
cat_W %>% dplyr::filter(GearGrp == "LL") %>%
  group_by(YearC, FlagName) %>% summarise(catch = sum(Qty_t)) %>% dplyr::filter(catch > 0) %>%
  ggplot(cat_W, aes(YearC, catch)) + facet_wrap(~FlagName) + 
  geom_point() + geom_line()


#### Catch - out
cat_out <- cat_W %>% mutate(Gear = ifelse(GearGrp == "LL", "LL", "OTH")) %>% group_by(YearC, Gear) %>% summarise(catch = sum(Qty_t)) %>%
  reshape2::acast(YearC ~ Gear, fill = 0)
write.csv(cat_out, "WSAI/WSAI_catch.csv")
...
cal_W_LL %>% group_by(YearC, ClassFrq3) %>% summarise(Nr = sum(Nr)) %>%
  ggplot(aes(ClassFrq3, Nr)) + facet_wrap(~ YearC, scales = "free_y") + 
  geom_point() + geom_line()

cal_W %>% dplyr::filter(GearGrpCode != "LL") %>% 
...
# CAL_out
cal_out <- cal_W %>% mutate(Gear = ifelse(GearGrpCode == "LL", "LL", "OTH")) %>% 
  group_by(YearC, ClassFrq3, Gear) %>% summarise(Nr = sum(Nr)) %>%
  reshape2::acast(YearC ~ Gear + ClassFrq3, fill = 0, aggregate = sum)
write.csv(cal_out, file = "WSAI/WSAI_CAL.csv")
