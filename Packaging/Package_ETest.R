...
ios = list()
names(ios) = ssnams
for(dd in 1:length(ssdirs))ios[[dd]] = all_ss3(paste0(ssfold,ssdirs[dd]))
lapply(ios,ss_names) # what are primary fleets / surveys
       
Fnams = list(c("F4_JPN_LL","F8_ESP_LL"),c("F1_EU_ESP","F2_JPN"),c("FS1_EU_SPN","FS2_BRA"),c("F_Tar_NZ","F_Tar_EU"))
Inams = list(c("S2_JPN_LATE","S4_EU_ESP"),c("S1_ESP-LL-N","S2_JP-LL-N"),c("BRA_index_TB2","JPN_index_TB2"),c("F_Tar_NZ","F_Tar_EU"))
