make_MOM = function(Name = "Simulated for AI training", nsim = 100, 
  ...
  MOM@CatchFrac = make_Catch_Frac(MOM)
  MOM
}
...
make_cpars = function(nstocks, nfleets, nsim, nyears, M_max = 0.6, M_min = 0.1, Linf_max = 1000, 
  ...
  for(ss in 1:nstocks){
    ...
    cpars_by_stock[[ss]] = add_M_K_L50_Linf(cpars_by_stock[[ss]], nsim, M_max, M_min, Linf_max, plot=plot)
  }
  
  for(ff in 1:nfleets){
    Effs = list()
    for(i in 1:nsim) Effs[[i]]= Stoch_effort(ny=nyears, plot=F, nstocks = nstocks, Ecor = Ecor)
    for(ss in 1:nstocks){
  ...
  ff=1
  for(ss in 1:nstocks)  cpars[[ss]][[1]]$D = add_dep(cpars_ss = cpars[[ss]])
